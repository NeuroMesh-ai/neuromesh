#!/usr/bin/env python3
"""
NeuroMesh v5.2.0 — Windows System Tray Launcher
Minimal system tray app for managing the NeuroMesh service.
...
def get_service_status() -> str:
    """Check if NeuroMesh service is running."""
    try:
...
def open_web_ui(icon, item):
    """Open NeuroMesh Web UI in default browser."""
    port = get_port()
...
def start_service(icon, item):
    subprocess.run(["sc", "start", "NeuroMesh"], capture_output=True)


def stop_service(icon, item):
    subprocess.run(["sc", "stop", "NeuroMesh"], capture_output=True)
...
def poll_status(icon: Icon):
    ...
    while icon.visible:
        ...
        icon.title = f"NeuroMesh — {'Running' if status == 'running' else 'Stopped'}"
        import time
        time.sleep(5)
...
def main():
    ...
    icon = Icon(
        ...
        title=f"NeuroMesh — {'Running' if status == 'running' else 'Stopped'}",
