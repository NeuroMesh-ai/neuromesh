; ============================================================================
; NeuroMesh v5.2.0 — Windows Installer (Inno Setup)
; P2P Distributed AI Network
; ============================================================================

#define AppName "NeuroMesh"
#define AppVersion "5.2.0"
#define Publisher "NeuroMesh-ai"
#define AppURL "https://github.com/NeuroMesh-ai/neuromesh"
...
DefaultDirName=C:\NeuroMesh
DefaultGroupName={#AppName}
UninstallDisplayName={#AppName} {#AppVersion}
UninstallDisplayIcon={app}\neuromesh.exe
OutputBaseFilename=NeuroMesh-Setup-{#AppVersion}
...
[Components]
Name: "main"; Description: "NeuroMesh Application"; Types: full custom; Flags: fixed
...
[Files]
; --- Application files ---
Source: "app\src\neuromesh_v5.py"; DestDir: "{app}\app\src"; Flags: ignoreversion; Components: main
...
Source: "bin\neuromesh-service.exe"; DestDir: "{app}"; Flags: ignoreversion; Components: service
Source: "neuromesh-service.xml"; DestDir: "{app}"; Flags: ignoreversion; Components: service

; --- Launcher ---
Source: "bin\neuromesh.exe"; DestDir: "{app}"; Flags: ignoreversion; Components: main

; --- Post-install script ---
Source: "post_install.py"; DestDir: "{app}\scripts"; Flags: ignoreversion; Components: main

; --- Tray launcher ---
Source: "neuromesh_tray.py"; DestDir: "{app}\scripts"; Flags: ignoreversion; Components: tray
...
Name: "startservice"; Description: "Start NeuroMesh service after installation"; GroupDescription: "Service:"; Flags: checkedonce
...
Filename: "{app}\neuromesh-service.exe"; \
    Parameters: "install"; \
    WorkingDir: "{app}"; \
    Flags: runhidden waituntilterminated; \
    StatusMsg: "Installing NeuroMesh service..."; \
    Components: service

; --- Configure service recovery ---
Filename: "sc.exe"; \
    Parameters: "failure NeuroMesh reset=0 actions= restart/30000/restart/60000/restart/120000"; \
    Flags: runhidden waituntilterminated; \
...
Filename: "sc.exe"; \
    Parameters: "config NeuroMesh start= auto"; \
    Flags: runhidden waituntilterminated; \
    Components: service

; --- Firewall rule (Private + Domain profiles only) ---
Filename: "netsh.exe"; \
    Parameters: "advfirewall firewall add rule name=""NeuroMesh P2P"" dir=in action=allow protocol=TCP localport={code:GetPort} profile=private,domain"; \
    Flags: runhidden waituntilterminated; \
...
; --- Start the service ---
Filename: "sc.exe"; \
    Parameters: "start NeuroMesh"; \
    Flags: runhidden waituntilterminated; \
    StatusMsg: "Starting NeuroMesh service..."; \
    Components: service; \
    Tasks: startservice

; --- Launch tray app ---
Filename: "{app}\python\python.exe"; \
    Parameters: "{app}\scripts\neuromesh_tray.py"; \
    WorkingDir: "{app}"; \
...
[UninstallRun]
; --- Stop service ---
Filename: "sc.exe"; Parameters: "stop NeuroMesh"; Flags: runhidden waituntilterminated

; --- Remove service ---
Filename: "{app}\neuromesh-service.exe"; Parameters: "uninstall"; Flags: runhidden waituntilterminated

; --- Remove firewall rule ---
Filename: "netsh.exe"; Parameters: "advfirewall firewall delete rule name=""NeuroMesh P2P"""; Flags: runhidden waituntilterminated
...
begin
  CustomPage := CreateCustomPage(
    ...
    'Configure your NeuroMesh node settings.'
  );
...
begin
  ...
  if Result = '' then
    Result := 'neuromesh-' + GetComputerNameString();
end;
...
begin
  ...
  begin
    MsgBox('NeuroMesh installation requires Administrator privileges.' + #13#10 +
...
begin
  ...
  begin
    if MsgBox('Delete all NeuroMesh data (configs, logs, conversations, memory)?',
