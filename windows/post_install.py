#!/usr/bin/env python3
"""
NeuroMesh v5.2.0 — Windows Post-Install Script
...
BASE_DIR = Path(os.environ.get("NEUROMESH_HOME", r"C:\NeuroMesh"))
...
def create_node_config(p2p_secret: str, port: int = 8080, node_name: str = None) -> dict:
    ...
    if node_name is None:
        import platform
        node_name = f"neuromesh-{platform.node().lower()[:16]}"
    ...
    config = {
        ...
        "security": {
            "tls_enabled": False,
            "tls_warning": "Running without TLS — do not expose to public internet without NEUROMESH_CERT/NEUROMESH_KEY",
        },
        "shared_models_dir": str(BASE_DIR / "shared_models"),
        "data_dir": str(BASE_DIR / "data"),
        "logging": {
            "level": "INFO",
            "file": str(BASE_DIR / "data" / "logs" / "neuromesh.log"),
        },
    }
    return config
...
def main():
    print("=" * 60)
    print("  NeuroMesh v5.2.0 — Post-Install Configuration")
    print("=" * 60)
    ...
    if not config["security"]["tls_enabled"]:
        ...
        print("   NeuroMesh is configured WITHOUT TLS encryption.")
        print("   Do NOT expose the P2P port to the public internet without setting")
        print("   NEUROMESH_CERT and NEUROMESH_KEY environment variables.")
        print("   Current config is safe for local/private network use only.")
        print()

    # Success
    print("=" * 60)
    print("  ✅ NeuroMesh v5.2.0 installed successfully!")
    print("=" * 60)
    ...
    print()
    print("  The NeuroMesh service will start automatically.")
    print("  Manage via system tray icon or: sc query NeuroMesh")
    print("=" * 60)
