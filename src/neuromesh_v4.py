#!/usr/bin/env python3
"""
🌐 NEUROMESH v4.2.0 — RÉSEAU P2P DISTRIBUÉ
===============================================
...
TOKEN_BLACKLIST_FILE = os.path.expanduser('~/.neuromesh/token_blacklist.json')
TOKEN_BLACKLIST_CLEANUP_INTERVAL = 3600  # Clean expired entries every hour

# LOW-03: TLS support
TLS_CERT_FILE = os.environ.get('NEUROMESH_CERT', '')
TLS_KEY_FILE = os.environ.get('NEUROMESH_KEY', '')
    ...
    def _parse_beacon(self, data: bytes, addr: tuple) -> Optional[Dict]:
        ...
        try:
            ...
            if isinstance(beacon_data, dict):
                ...
                if payload.get('type') != 'neuromesh_discovery':
                    return None
            ...
            if proc.returncode == 0:
                ...
                for peer in status.get('Peer', {}).values():
                    if peer.get('Online', False):
                        ...
                        # NeuroMesh nodes are expected to have recognizable names
                        # This prevents random Tailscale peers from being auto-discovered
                        # as NeuroMesh nodes (unless explicitly allowed)
                        is_neuromesh = (
                        ...
                        if not allowed_peers and not is_neuromesh:
                            continue
...
async def main():
    ...
    if TLS_CERT_FILE and TLS_KEY_FILE and os.path.isfile(TLS_CERT_FILE) and os.path.isfile(TLS_KEY_FILE):
        ...
        if brain.host != '127.0.0.1' and brain.host != 'localhost':
            logger.warning("⚠️  LOW-03: Running without TLS on public interface — set NEUROMESH_CERT and NEUROMESH_KEY")
...
if __name__ == '__main__':
    ...
    logger.info(f"Starting NeuroMesh v4.1.5 as '{node}'")
    asyncio.run(main())